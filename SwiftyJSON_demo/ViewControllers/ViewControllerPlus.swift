...
let fetchBatchSize = 20 // commit records
...
class ViewController: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let githubCommit = fetchedResultsController.object(at: indexPath)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let vController = storyboard?.instantiateViewController(withIdentifier: "Detail") as? DetailViewController {
            vController.detailItem = fetchedResultsController.object(at: indexPath)
    ...
    override func tableView(_ tableView: UITableView,
        ...
        if editingStyle == .delete {
            let commit = fetchedResultsController.object(at: indexPath) // selected record
    ...
    // deletes the row in tableView (uses delegate)
    func controller(_ controller: NSFetchedResultsController<NSFetchRequestResult>,
        ...
        switch type {
        case .delete:
            tableView.deleteRows(at: [indexPath!], with: .automatic) // .automatic is an animation mode
    ...
    func loadRecordsFromDatabase() {
        if fetchedResultsController == nil { // create it
            let request = Commit.createFetchRequest()
            request.sortDescriptors = [NSSortDescriptor(key: "author.name", ascending: true),
        ...
        fetchedResultsController.fetchRequest.predicate = githubCommitPredicate

        do {
            try fetchedResultsController.performFetch()
            tableView.reloadData()
        } catch {
            fatalError("Error fetching records from CoreData database failed")
        }
    }

    func saveRecordsToDatabase() {
        let context: NSManagedObjectContext = container.viewContext
        if context.hasChanges {
            do {
                try context.save()
                print("Succesfully saved record(s) to CoreData database")
            } catch {
